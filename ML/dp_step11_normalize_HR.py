...
full_filename = os.path.join(DATA_DIR, "HR_all.csv")
...
def addPilot(new_df, df):
    ...
    df['HR'] = df.apply(lambda row: normilize(HR_min, HR_max,
                                                 row['Heart Rate']), axis=1)
...
new_df = new_df[['date', 'run', 'timeInterval', 'HR']]

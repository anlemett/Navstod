...
et_df = pd.read_csv(os.path.join(DATA_DIR, "et_stats_per_slot.csv"), sep=' ', dtype={'date':str})
...
for key in runs_dict:
    ...
    for run in runs:
        ...
        run_df = pd.merge(et_date_df, ch_date_df, how="inner", on=["date", "run", "timeInterval"])
        
        #print(et_run_df.head(1))
        #print(ch_run_df.head(1))
        #print(run_df.head(1))
